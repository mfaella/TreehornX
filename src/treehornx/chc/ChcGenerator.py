...
from chc.ChcBuilder import ChcBuilder
...
class ChcGenerator:
    ...
    def generate(self) -> Iterable[FNode]:
        db = PairDB()
        stepper = Stepper(self.function, self.k, self.m, self.n)
        chcbuilder = ChcBuilder(self.function.vars, self.root.sort.fields)
        ...
        while queue:
            ...
            if pair in processed:
                continue
            processed.add(pair)
            tau_b, tau_b_false, step_kind = stepper.step(pair)
