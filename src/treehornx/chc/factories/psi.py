...
from treehornx.chc.factories.PreFactory import PsiType
from treehornx.enum_labels.core.Label import Label

def psi_sll_sorted(children_states: dict[str|int, dict[str, FNode]|None], fields: dict[str, FNode], states: dict[str, FNode]) -> FNode:
    if children_states['next'] is None:
        return smt.Equals(states['data'], fields['data'])

    return smt.And(
        smt.GE(children_states['next']['data'], fields['data']),
        smt.Equals(states['data'], fields['data'])
    )

def psi_sll_sorted_strict(children_states: dict[str|int, dict[str, FNode]|None], fields: dict[str, FNode], states: dict[str, FNode]) -> FNode:
    if children_states['next'] is None:
        return smt.Equals(states['data'], fields['data'])

    return smt.And(
        smt.GT(children_states['next']['data'], fields['data']),
        smt.Equals(states['data'], fields['data'])
    )

def psi_bst(children_states: dict[str|int, dict[str, FNode]|None], fields: dict[str, FNode], states: dict[str, FNode]) -> FNode:
...
def psi_bst_strict(children_states: dict[str|int, dict[str, FNode]|None], fields: dict[str, FNode], states: dict[str, FNode]) -> FNode:
    left_state = children_states['left']
    right_state = children_states['right']
...
def _psi_avl_template(children_states: dict[str|int, dict[str, FNode]|None], fields: dict[str, FNode], states: dict[str, FNode], bst_psi: PsiType) -> FNode:
    ...
    if left_state is None and right_state is None:
        ...
        constraints.append(
            smt.And(
                smt.GE(diff, smt.Int(-1)),
                smt.LE(diff, smt.Int(1))
            )
        )
...
def psi_avl(children_states: dict[str|int, dict[str, FNode]|None], fields: dict[str, FNode], states: dict[str, FNode]) -> FNode:
    return _psi_avl_template(children_states, fields, states, psi_bst)

def psi_avl_strict(children_states: dict[str|int, dict[str, FNode]|None], fields: dict[str, FNode], states: dict[str, FNode]) -> FNode:
